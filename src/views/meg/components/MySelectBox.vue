...
function initSelector() {
  if (props.modelValue) {
    const target = props.options.find(
      (option) => option[props.value] === props.modelValue,
    )
    if (target) {
...
function openSelect() {
  ...
  if (!hoverValue.value && selectedName.value) {
    const target = props.options.find(
      (option) => option[props.label] === selectedName.value,
    )
...
<template>
  <div class="m-select" :style="`width: ${selectWidth}; height: ${height}px;`">
    <div
      :class="[
        'm-select-wrap',
        { hover: !disabled, focus: showOptions, disabled: disabled },
      ]"
      tabindex="1"
      ref="selectRef"
    ...
    >
      ...
      <span
        :class="[
          'u-select-item',
          { 'select-item-gray': !selectedName || showOptions },
        ]"
    ...
    <TransitionGroup name="fade" tag="div">
      <div
        ...
        @mouseleave="onLeave"
        key="1"
        :style="`top: ${height + 4}px; line-height: ${
          height - 10
        }px; max-height: ${maxDisplay * height + 9}px; width: 100%;`"
      >
        <p
          ...
          @click="
            option.disabled
              ? () => false
              : onChange(option[value], option[label], index)
          "
        >
          {{ option[label] }}
        </p>
...
.focus {
  // 激活时样式
  border-color: skyblue;
  box-shadow: 0 0 0 2px fade(skyblue, 20%);
}
.disabled {
  // 下拉禁用样式
  color: rgba(0, 0, 0, 0.25);
